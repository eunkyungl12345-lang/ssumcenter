...
module.exports = async function handler(req, res) {
  ...
  if (body.action === "addfields") {
    ...
    const tbl = (meta.tables || []).find(t => t.name === tableName);
    if (!tbl) return res.status(404).json({ error: "테이블 없음: " + tableName });
    ...
    for (const name of (body.fields || [])) {
      ...
      if (!r.ok) return res.status(r.status).json({ error: name + " 실패: " + (d.error?.message || JSON.stringify(d.error)), added });
      added.push(name);
    }
